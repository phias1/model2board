...
<section class="content">
	...
	파일이 업로드되는 폼은 반드시 method는 post로
	...
	<form id="registerform" enctype="multipart/form-data" method="post"
		...
		<table border="1" width="50%" height="80%" align='center'>
			...
			<tr>
				...
				<td>&nbsp;&nbsp;&nbsp; 
				<!-- onblur는 포커스가 떠날 때 confirmId()를 호출해서 email 중복 검사를 수행한다. -->
...
function confirmId(){
	...
	$.ajax({
		...
		success:function(data){
			//이메일이 중복되지 않은 경우
			if(data.result == true){
				emaildiv.innerHTML = "사용 가능한 이메일"
				emaildiv.style.color = 'green';
				emailcheck = true;
			}
			//이메일이 중복된 경우
			else{
				emaildiv.innerHTML = "사용 불가능한 이메일"
...
//image에서 선택이 변경되었을 때 호출되는 함수만들기
...
function readURL(input){
	...
	//마지막 3글자가 gif도 아니고 jpg도 아니고 png도 아니면
...
//false를 리턴하면 서버로 전송되지 않는다.
function check(){
	//emailcheck의 값이 false 이면 서버로 전송하지 않도록
	if(emailcheck == false){
		document.getElementById("emailDiv").innerHTML = "이메일 중복 검사를 통과하지 못함";
	...
	if(pw.value != pwconfirm.value){
		alert("두개의 비밀번호는 일치해야 합니다.");
		pw.focus();
		return false;
	}
	//비밀번호는 숫자, 영문자, 특수문자 1개이상으로 8자 이상 만들어졌는지 검사
	//정규식 이용 - 숫자, 영문자, 특수문자
	var p1 = /[0-9]/;
    var p2 = /[a-zA-Z]/;
    var p3 = /[~!@#$%^&*()]/;
    if(!p1.test(pw.value) || !p2.test(pw.value) || !p3.test(pw.value) || pw.value.length < 8){
		alert("비밀번호는 8자리 이상 숫자, 영문자, 특수문자를 포함해야 합니다.");
		pw.focus();
	    return false;
	}
}
